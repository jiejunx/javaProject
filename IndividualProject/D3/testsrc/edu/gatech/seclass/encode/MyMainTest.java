...
public class MyMainTest {
    ...
    */
    ...
    private static final String FILE10 = "";
    private static final String FILE11 = "abcd\n\n";
    ...
    // Purpose: Test Case 31, catch bug 9, can not duplicate letters for -c method
    // bug 9: if using -c to covert letters, we can not use same 
    // letter input as string two times, since it will make the function useless 
    @Test
    public void mymainTest31() throws Exception {
        File inputFile31 = createInputFile(FILE10);
        String args[]  = {"-c", "Aa", inputFile31.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: catch bug 7, use -n method for two times without following an integer
    @Test
    public void mymainTest32() throws Exception {
        File inputFile31 = createInputFile(FILE8);
        String args[]  = {"-n", "-n", inputFile31.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: catch bug 3, rotate letters for two times in the same 
    // direction, interger added together is same as the length of the letter
    @Test
    public void mymainTest33() throws Exception {
        File inputFile33 = createInputFile(FILE8);
        String args[]  = {"-l", "2", "-l","4", inputFile33.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: catch bug 6: there should be a encode method in the command, instead of only 
    // integer inputs
    @Test
    public void mymainTest34() throws Exception {
        File inputFile34 = createInputFile(FILE1);
        String args[]  = {"5", "-1", inputFile34.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: Bug 4, for -d method, inputfile can not be empty
    @Test
    public void mymainTest35() throws Exception {
        File inputFile34 = createInputFile(FILE10);
        String args[]  = {"-d", "1", inputFile34.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: Bug 5, for -d method, the command after it should be 
    // integer value instead of letters
    @Test
    public void mymainTest36() throws Exception {
        File inputFile36 = createInputFile(FILE10);
        String args[]  = {"-d", "three", inputFile36.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: Bug 2, for -c method, there should be
    // a letter string as input, instead of an empty string.
    @Test
    public void mymainTest37() throws Exception {
        File inputFile37 = createInputFile(FILE7);
        String args[]  = {"-c", "", inputFile37.getPath()};
        Main.main(args);

        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: Bug 10, if the input string has two empty rows\n\n,
    // when using rotate method, it only rotate rows with string values 
    @Test
    public void mymainTest38() throws Exception {
        File inputFile38 = createInputFile(FILE11);
        String args[]  = {"-l", "1", inputFile38.getPath()};
        Main.main(args);
        String expected38 = "bcda\n\n";
    ...
    // Frame #: Test Case 2, corner case, my assumption was run -c first, then -l, then -n
    // Failure Type: Test Failure”. error in my encode
    @Test
    public void mymainTest2() throws Exception {
        ...
        Main.main(args);
        String expected16 = "32020307083031";//06020307080405 why??
    ...
    // Frame #: Test Case 6, catch bug 1, since input integer can not be 0 for -l or -r method
    @Test
    ...
    // Frame #: Test Case 8, catch Bug 8, input integer can not be 
    // over 25 for -n method
    @Test
    ...
    // Frame #: Test Case 9, catch Bug 8, input integer can not be 
    // smaller than 0 for -n method
    @Test
