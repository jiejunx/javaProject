...
public class MyMainTest {
    ...
    */
    ...
    // Purpose: Test Case31, check if int<0
    // Frame #: Test Case 1
    @Test
    public void mymainTest31() throws Exception {
        File inputFile1 = createInputFile(FILE1);
        String args[]  = {"-d", "-1", inputFile1.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }

    // Purpose: Test Case32, check if int<0
    // Frame #: Test Case 32
    // New Failure Type: BUG: <short explanation of the failure and what you think it’s the corresponding bug>”.
    @Test
    public void mymainTest32() throws Exception {
        File inputFile1 = createInputFile(FILE1);
        String args[]  = {"-d", "hh", inputFile1.getPath()};
        Main.main(args);
        assertEquals("Usage: encode [-n [int]] [-r int | -l int] [-c string] [-d int] <filename>", errStream.toString().trim());
    }
